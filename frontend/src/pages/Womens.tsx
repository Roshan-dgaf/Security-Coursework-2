...
const Womens = () => {
  ...
  // Filter to only show Female category, case-insensitive
  ...
  // Apply type filter only on female products
  const filteredProducts = useMemo(() => {
    if (activeFilter === "All") return femaleProducts;
    if (activeFilter === "Others") {
      return femaleProducts.filter(
  ...
  }, [activeFilter, femaleProducts]);

  return (
    <div className="flex w-full px-4 py-6 font-sans">
      <ProductSidebar />
      <main className="flex-1">
        <h1 className="text-2xl font-bold mb-6">Cat's Collection</h1>
        ...
        {isLoading ? (
          ...
          <div className="w-full flex justify-center py-20">
            <span className="text-gray-400 text-lg">No Cat's products found.</span>
          </div>
        ) : (
