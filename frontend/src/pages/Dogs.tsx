...
const Dogs = () => {
    ...
    // Filter to only show Male category (representing Dogs), case-insensitive
    ...
    // Apply type filter only on dog products
    const filteredProducts = useMemo(() => {
        if (activeFilter === "All") return dogProducts;
        if (activeFilter === "Others") {
            return dogProducts.filter(
            ...
            <main className="flex-1">
                ...
                {isLoading ? (
                    <div className="w-full flex justify-center py-20">
                        <span className="text-gray-400 text-lg">Loading dogs...</span>
                    </div>
                ) : isError ? (
                    <div className="w-full flex justify-center py-20">
                        <span className="text-red-500 text-lg">Failed to load dogs.</span>
                    </div>
                ) : filteredProducts.length === 0 ? (
                    <div className="w-full flex justify-center py-20">
                        <span className="text-gray-400 text-lg">No dogs available for adoption.</span>
                    </div>
                ) : (
